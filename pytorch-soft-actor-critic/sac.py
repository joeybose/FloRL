...
class SAC(object):
    def __init__(self, num_inputs, action_space, args):
        ...
        if self.policy_type == "Gaussian" or self.policy_type == "Exponential" or self.policy_type == "LogNormal" or self.policy_type == "Laplace":
            ...
            if args.flow_model == 'made':
            ...
            elif args.flow_model == 'maf-2':
                if args.gaussian_encoder:
                    state_enc = GaussianEncoder(self.num_inputs, self.action_space,\
                            args.hidden_size,args).to(device)
                else:
                    state_enc = StateEncoder(self.num_inputs,self.action_space,args.hidden_size,args.num_state_out).to(device)
                modules = [state_enc]
                ...
                self.policy = fnn.FlowSequential(*modules).to(device)
    ...
    def update_parameters(self, state_batch, action_batch, reward_batch, next_state_batch, mask_batch, updates):
        ...
        with autograd.detect_anomaly():
            ...
            # Regularization Loss
            if self.policy_type == "Gaussian" or self.flow_model == 'planar' or self.policy_type == "Exponential" or self.policy_type == "LogNormal" or self.policy_type == "Laplace":
